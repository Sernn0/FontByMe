#!/usr/bin/env python3
"""
Pipeline v3 - Binarized Handwriting Model

Uses Style Encoder and Decoder trained on binarized handwriting data.
"""
...
MODEL_DIR = ROOT / "runs" / "handwriting_v3"
...
def generate_all_glyphs_v3(
    style_images: List[Path],
    output_dir: Path,
) -> List[Path]:
    """Generate all 2350 Korean glyphs using v3 (binarized handwriting) model."""
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    # Load models
    print(f"[Pipeline v3] Loading Style Encoder: {MODEL_DIR / 'style_encoder_best.h5'}")
    style_encoder = keras.models.load_model(MODEL_DIR / "style_encoder_best.h5", compile=False)

    print(f"[Pipeline v3] Loading Decoder: {MODEL_DIR / 'decoder_best.h5'}")
    decoder = keras.models.load_model(
        MODEL_DIR / "decoder_best.h5",
    ...
    # Extract style from sample images
    max_samples = 5
    samples_to_use = style_images[:max_samples]
    print(f"[Pipeline v3] Extracting style from {len(samples_to_use)} samples (of {len(style_images)} total)...")
    ...
    for img_path in samples_to_use:
        ...
        style_vec = style_encoder(img_batch, training=False)
        style_vecs.append(style_vec.numpy())
    ...
    print(f"[Pipeline v3] Style vector shape: {avg_style.shape}")
    ...
    print(f"[Pipeline v3] Generating {len(korean_chars)} characters...")
    ...
    for start in range(0, n, batch_size):
        ...
        preds = decoder([c_vecs, s_vecs], training=False).numpy()
        ...
        if (end % 500 == 0) or (end == n):
            print(f"[Pipeline v3] Generated {end}/{n} characters")
