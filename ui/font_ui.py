...
# 파이프라인 모듈 (v3 - 이진화 손글씨 학습 모델)
PIPELINE_AVAILABLE = False
try:
    from src.inference.pipeline_v3 import generate_all_glyphs_v3
    from src.inference.pipeline import create_ttf_font  # TTF 생성은 기존 것 사용
    PIPELINE_AVAILABLE = True
    print("[UI] Pipeline v3 loaded successfully!")
except ImportError as e:
    print(f"[UI] Pipeline v3 import failed: {e}")
...
class App(TkinterDnD.Tk):
    ...
    def _run_model_stub(self, processed_images: List[Path]) -> List[Path]:
        ...
        # Generate all glyphs using v3 pipeline (binarized handwriting model)
        print(f"[UI] Calling generate_all_glyphs_v3 with {len(processed_images)} style images")
        glyph_dir = work_dir / "glyphs"
        try:
            glyph_paths = generate_all_glyphs_v3(
